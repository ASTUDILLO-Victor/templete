...
$sql = "SELECT MIN(fecha_hora) AS start_time, MAX(fecha_hora) AS end_time FROM lectura_mq138";
...
while ($current_time <= $end_time) {
    ...
    if ($prom_valor !== null) {
        ...
        $check_stmt = $conn->prepare("
            SELECT COUNT(*) FROM prom_mq138 WHERE hora_inicio = ?
        ");
        if (!$check_stmt) {
        ...
        if ($count > 0) {
            ...
            $update_stmt = $conn->prepare("
                UPDATE prom_mq138 SET prom_valor = ?, estado = ? WHERE hora_inicio = ?
            ");
            if (!$update_stmt) {
                die("Error en la preparación de la consulta de actualización: " . $conn->error);
            }
            $update_stmt->bind_param("dss", $prom_valor, $estado, $current_time_str);
            $update_stmt->execute();
            $update_stmt->close();
        } else {
            // Insertar un nuevo registro
            $insert_stmt = $conn->prepare("
                INSERT INTO prom_mq138 (prom_valor, hora_inicio, estado)
                VALUES (?, ?, ?)
            ");
            if (!$insert_stmt) {
                die("Error en la preparación de la consulta de inserción: " . $conn->error);
            }
            $insert_stmt->bind_param("dss", $prom_valor, $current_time_str, $estado);
            $insert_stmt->execute();
            $insert_stmt->close();
        }
    }
