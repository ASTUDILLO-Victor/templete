...
$sqlAll = "SELECT id, action, timestamp, descripcion FROM relay_log ORDER BY timestamp DESC";
$queryAll = $pdo->prepare($sqlAll);
$queryAll->execute();
$allData = $queryAll->fetchAll(PDO::FETCH_ASSOC);

// Consulta para obtener el último dato del sensor
$sqlLast = "SELECT id, action, timestamp, descripcion FROM relay_log ORDER BY timestamp DESC LIMIT 1";
